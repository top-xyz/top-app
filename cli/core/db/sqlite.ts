...
export class SQLiteDB {
  ...
  private async init() {
    ...
    this.db.exec(`
      ...
      CREATE INDEX IF NOT EXISTS idx_project_contexts_project_id 
      ON project_contexts(project_id);

      CREATE INDEX IF NOT EXISTS idx_project_contexts_category 
      ON project_contexts(project_id, category);

      CREATE INDEX IF NOT EXISTS idx_context_relationships 
      ON context_relationships(source_context_id, target_context_id);
    `);
  }
...
const DB_PATH = path.join(process.cwd(), 'data', 'top.db');
